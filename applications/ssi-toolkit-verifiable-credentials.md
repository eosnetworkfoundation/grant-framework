...
Allow any EOSIO account to issue, hold and verify Verifiable Credentials signed by its EOSIO account key(s)
...
The self-sovereign identity (SSI) toolkit allows software developers to use self-sovereign identity in their EOSIO applications. This open-source typescript library can be used on public or private EOSIO chains and is consumed by wallets and front-end clients to provide SSI functions with EOSIO [DIDs](https://w3c.github.io/did-core/). The kit aims to have two main components:

1. Verifiable credentials - this allows any EOSIO account to create, hold and verify [verifiable credentials](https://www.w3.org/TR/vc-data-model) (VC). This can be used with personal information, medical records, certificates or any other data. It can be used and verified with another account.
...
This is an _example_ showing how you can use this library to issue a Verifiable Credential signed by the &quot;active&quot; key of an EOSIO account. The final implementation interface may differ, but will have the same functionality.
...
See
[https://github.com/Tonomy-Foundation/EOSIO-SSI-Toolkit](https://github.com/Tonomy-Foundation/EOSIO-SSI-Toolkit)

#### **Relationship to EOSIO**

This project leverages the EOSIO accounts and the EOSIO DID method to sign verifiable credentials. It is a core building block for identity applications on top of all EOSIO blockchains, public and private. This software can be used by any EOSIO application to send VCs privately among accounts on the same EOSIO chain, and also between separate EOSIO chains, and also to any other chain or non-chain with a DID method (see [list here](https://www.w3.org/TR/did-spec-registries/#did-methods)).

#### **Reason for Interest**

We believe that these components have a strategic technical value for the EOSIO ecosystem and are sorely missing. This case is outlined in the Core+ paper.

To add to what is already written in Core+, SSI brings a number of strong advantages for apps built on EOSIO:
...
3. SSI adoption has been encouraged and invested in heavily by enterprise companies like IBM and Microsoft as well as the EU, the US and Canada. They have funded or contributed strongly to the development of these technologies and thus likely adopters and evangelists of the technology. This gives validity to the technology and a trusted set of technologies to EOSIO chains and applications. It also puts these large-scale enterprises in a strong position to adopt technology, which due to its interoperable nature can be leveraged by EOSIO applications.
4. Standardised and well-adopted tooling to allow data standards and transfer across the Internet. EOSIO apps can benefit a significant amount of existing SSI tooling that already exists. The DID standard has [moved to a W3C Recommendation](https://www.w3.org/2022/06/DIDRecommendationDecision.html) as of June 30 2022 indicating a serious move towards it becoming an approved Internet standard.
5. Taking identity and other credentials off the blockchain and allowing clients to share it directly is a much more scalable strategy. This allows applications and EOSIO chains to scale better.
...
This typescript library will use the [EOSIO-DID resolver](https://github.com/Gimly-Blockchain/eosio-did-resolver) to fetch DID documents for EOSIO accounts and use the cryptographic material (in JWK format) to verify the credential.

We will use an existing Verifiable Credential library to issue and verify the credential. To accomplish this we will need to create an implementation of the [Verifiable Conditions W3C library](https://github.com/w3c-ccg/verifiable-conditions), which is used in the EOSIO DID to express the multiple keys in an account. This implementation will take the form of a software module that is imported into the VC library.

Several VC libraries exist, and we will choose the best library for the purpose of signing a VC with an EOSIO account&#39;s key. Existing libraries include:
...
**Identity and accountability** - There is currently a lack of identity solutions in EOSIO. This refers to a solution in which information can be shared which identifies the user of an account. This lack of identity solutions leads to accountability problems. It can be seen on democratic apps like Eden, Pomelo and in marketplace scams like on Atomic Hub with NFT sales.
...
In the coming few years, expect to see more and more adoption of VCs in identity solutions. Ensuring that EOSIO is able to participate in this identity movement is critical to ensure it stays at pace with the latest identity technology.

EOSIO Verifiable credentials start to align the EOSIO ecosystems in these technologies so that it is ready for these enterprise and state-level systems to exist. These should be leveraged due to their interoperable data standards in EOSIO applications.
...
The work to bring SSI to EOSIO as being ongoing for several years. Jack Tanner, as a contractor and then employee of Gimly has led the work to create and market the EOSIO DID. This has been a team effort with Caspar Roelofs, the director of Gimly.
...
This project is the next and most important step in this SSI journey for EOSIO (Verifiable Credentials are the primary use case of SSI).
...
| 1. | Environment setup | We will set up a developer environment which includes a running EOSIO node that can be used during unit test execution. |
| 2. | Decision for Verifiable Credential library | We will evaluate the options available for use of an existing verifiable credentials library that can best support the EOSIO protocol. This may happen during step 3 and 4. Results of this discussion will be published in a Github issue. |
| 3. | Implementation of W3C Vrifiable Condition | This will require us to create a plug-in to the verifiable credential library so that it can support the W3C verifiable conditions Standard as part of the EOSIO DID - allowing the VC to be signed by any permission inside an EOSIO account. |
| 4. | Implementation of issue() function | We will implement the issue() function that signs a VC with the chosen permission of the EOSIO account. It can be signed by one key, several keys, or a delegated permission. |
