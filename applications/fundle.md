...
#### Open source 
The blue parts from the application architecture are already finished, these parts won't be open source since it is custom made for the Fundle platform! This part has no value for the community to be open source since there is no innovative logic which can have benefits for the ENF community. The token bonding curve mechanism (so everything that is build for the blockchain part of Fundle) will be open source and usable for the EOS community. They can make use of and learn from the mechanism we will build. We will not use the budget of this grant for the non-open source part of Fundle. 
