...
This grant will be used to build the community token model, integrate with TrustEVM and launch the platform. 
...
#### Open source 
The blue parts of the application architecture picture above are already finished, these parts won't be open source since it is custom made for the Fundle platform! This part has no value for the community to be open source since there is no specific logic which can have benefits for the ENF community. The token bonding curve mechanism (so everything that is build for the blockchain part of Fundle) will be open source and usable for the EOS community. They can make use of and learn from the mechanism or build and manage their communities on the Fundle platform!

#### Community token mechanism (Open Source)
With this grant we will build the community token model (a token bonding curve mechanism) which will thereafter be used for the Fundle application. We will build further up on the Balancer protocol. The protocol will allow users to buy/sell tokens per community according to a token bonding curve mechanism. So each community has its own token which is created when business create their community on Fundle. The reserve token will be a stablecoin (e.g. USDC,USDT). Per community the stablecoin will be bonded to the community token. When users send the stablecoin, the community token will be mint according to the current price and supply and when users want to sell their tokens the tokens are burned according to the current price and supply. The price of a community token will be determined by the $community supply and the holding patterns of token holders.
...
- Where and how does your project fit into the ecosystem?
  ...
  Our platform will run on the EOS blockchain, this brings lots of new users (also non/new crypto users) who will interact and get to know EOS. The contracts of Fundle will be deployed on TrustEVM. Fundle shows others what possibilities TrustEVM has and that it is possible to build these platforms on the EOS blockchain! We can be a perfect use case for TrustEVM. The purpose of Fundle is to have lots of interaction between members and the business. This will result in many transactions on the EOS Blockchain. Furthermore, communities can be build and managed via the Fundle platform!
...
| -----: | ----------- | ------------- |
| 0a. | Smart Contracts | The Balancer protocol will be forked and the contracts will be customized for the community token model. This part of the application will be open source. |
