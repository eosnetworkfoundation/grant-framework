...
- https://www.npmjs.com/settings/trustevm/packages , we have published an SDK to npmjs.com, this will make it easy for other developers to connect to our smart contract. There are multiple use cases for this, for instance market makers and arbitragers can use our SDK to make their systems work. 
  - TrustSwap will not divert far from the orignal code source, instead adapt to TrustEVM the open source forked project. We start with the open source version of the Uniswap V2, and will upgrade to V3 once the license allow is. 
...
- The project solves the need of users to trade among multiple crypto currencies; alsothe need to obtain tokens of new projects and swap for stable currencies. 
-  There are multiple crypto decentralized exchanged on out ecosystem, yet we are the first one available on the upcoming Trust EVM EOS blockchain. There are currently no other decetralized exchanges built for Trust EVM.
...
We have a fully functional decentralized exchange working on the TrustEVM testmet, we have also deploued a functipon staking farm. Both are being actively tested by the EOS community. Further we have deployed multisign bridge conrtacts, a faucet, several EVM20 tokens, wrapped EVM20 contract, and Multicall contract. We are the first team deploying contracts and actively testing the TrustEVM. 
